...
@section('page_title', __('Edit landlord account'))

@section('page_name')
    <b class="text-uppercase">{{ __('Edit landlord account') }}</b>
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item">{{ __('User Management') }}</li>
    <li class="breadcrumb-item">
        <a href="{{ route('administration.landlords.index') }}">{{ __('Landlord') }}</a>
    </li>
...
@section('custom_css')
    <style>
        ...
        .landlord-account-edit [class*="col-"] {
            min-width: 0;
        }
        .landlord-account-edit .bootstrap-select {
            width: 100% !important;
        }
        .landlord-account-edit .bootstrap-select > .dropdown-toggle {
            text-align: start;
        ...
        .landlord-account-edit .bootstrap-select > .dropdown-toggle:hover,
        .landlord-account-edit .bootstrap-select > .dropdown-toggle:focus,
        .landlord-account-edit .bootstrap-select.show > .dropdown-toggle {
            background-color: #fff !important;
            color: #6f6b7d !important;
        ...
        .landlord-account-edit .bootstrap-select.is-invalid > .dropdown-toggle {
...
@section('content')
    <div class="row justify-content-center landlord-account-edit">
        <div class="col-12 col-xl-10">
            <div class="card mb-4" id="landlord-edit-card">
                <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
                    <h5 class="mb-0">{{ __('Edit landlord account') }}</h5>
                    <span class="text-muted small">{{ __('User ID') }}: <span class="text-heading fw-medium">{{ $user->userid }}</span></span>
                </div>
                <div class="card-body">
                    <form action="{{ route('administration.landlords.update', ['user' => $user]) }}" method="post" id="landlord-edit-form" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="billing_address" id="billing_address" rows="3"
                                    ...
                                    placeholder="{{ __('Street, city, postal code') }}">{{ old('billing_address', $user->billing_address) }}</textarea>
            ...
            $(function () {
                ...
                if ($status.length && $.fn.selectpicker) {
                    $status.selectpicker({
                        liveSearch: false,
                        width: '100%',
                        container: '#landlord-edit-card',
                    });
                }
            });

            var formEl = document.getElementById('landlord-edit-form');
            if (formEl) {
