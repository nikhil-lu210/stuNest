...
@section('page_title', __('Agent profile'))
...
@section('page_name')
    <b class="text-uppercase">{{ __('Agent profile') }}</b>
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item">{{ __('User Management') }}</li>
    <li class="breadcrumb-item">{{ __('Agent') }}</li>
    <li class="breadcrumb-item">
        <a href="{{ route('administration.agents.index') }}">{{ __('All Agents') }}</a>
    </li>
                ...
                <div class="user-profile-header d-flex flex-column flex-sm-row text-sm-start text-center mb-4">
                    ...
                    <div class="flex-grow-1 mt-3 mt-sm-5">
                        <div class="d-flex align-items-center justify-content-md-between justify-content-start mx-4 flex-md-row flex-column gap-4">
                            ...
                            @can('User Update')
                                <a href="{{ route('administration.agents.edit', ['user' => $user]) }}" class="btn btn-primary waves-effect waves-light">
    ...
    <div class="row">
        <div class="col-md-12">
            <ul class="nav nav-pills flex-column flex-sm-row mb-4">
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('administration.agents.show') ? 'active' : '' }}" href="{{ route('administration.agents.show', ['user' => $user]) }}">
                        <i class="ti-xs ti ti-user-check me-1"></i>
                        {{ __('Profile') }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('administration.agents.show.applications') ? 'active' : '' }}" href="{{ route('administration.agents.show.applications', ['user' => $user]) }}">
                        <i class="ti-xs ti ti-file-text me-1"></i>
                        {{ __('Applications') }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('administration.agents.show.favorites') ? 'active' : '' }}" href="{{ route('administration.agents.show.favorites', ['user' => $user]) }}">
