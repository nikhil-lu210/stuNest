...
@section('page_title', __('Create agent account'))

@section('page_name')
    <b class="text-uppercase">{{ __('Create agent account') }}</b>
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item">{{ __('User Management') }}</li>
    <li class="breadcrumb-item active">{{ __('Create agent account') }}</li>
@endsection
...
@section('custom_css')
    <style>
        ...
        .agent-account-create [class*="col-"] {
            min-width: 0;
        }
        /* Contain stray horizontal overflow; Bootstrap Select menu uses container: #agent-create-card */
        .agent-account-create .bootstrap-select {
            width: 100% !important;
        }
        /* Match .form-control (Vuexy): Bootstrap Select’s .btn toggle defaults to grey — force same surface as text inputs */
        .agent-account-create .bootstrap-select > .dropdown-toggle {
            text-align: start;
        ...
        .agent-account-create .bootstrap-select > .dropdown-toggle:hover,
        .agent-account-create .bootstrap-select > .dropdown-toggle:focus,
        .agent-account-create .bootstrap-select.show > .dropdown-toggle {
            background-color: #fff !important;
            color: #6f6b7d !important;
        ...
        .agent-account-create .bootstrap-select.is-invalid > .dropdown-toggle {
...
@section('content')
    <div class="row justify-content-center agent-account-create">
        <div class="col-12 col-xl-10">
            <div class="card mb-4" id="agent-create-card">
                <div class="card-header">
                    <h5 class="mb-0">{{ __('Create agent account') }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('administration.agents.store') }}" method="post" id="agent-create-form" novalidate>
                        @csrf
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                {{-- Bootstrap Select (Vuexy): styled dropdown, no search; no-select2 skips global Select2 init --}}
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="office_address" id="office_address" rows="3"
                                    ...
                                    placeholder="{{ __('Street, city, postal code') }}">{{ old('office_address') }}</textarea>
            ...
            $(function () {
                ...
                if ($status.length && $.fn.selectpicker) {
                    $status.selectpicker({
                        liveSearch: false,
                        width: '100%',
                        container: '#agent-create-card',
                    });
                }
            });

            /**
             * Password visibility: same capture-phase handler as student create (avoids double-toggle with Vuexy Helpers.initPasswordToggle).
             */
            var formEl = document.getElementById('agent-create-form');
            if (formEl) {
