@extends('administration.user-management.agent.show')
...
@section('profile_content')
    <div class="row justify-content-center">
        ...
        <div class="col-md-6">
            <div class="card mb-4">
                <div class="card-body">
                    <small class="card-text text-uppercase">{{ __('Agency') }}</small>
                    <dl class="row mt-3 mb-1">
                        <dt class="col-sm-4 mb-2 fw-medium text-nowrap">
                            <i class="ti ti-building text-heading"></i>
                            <span class="fw-medium mx-2 text-heading">{{ __('Agency name') }}</span>
                        </dt>
                        <dd class="col-sm-8">{{ $user->agency_name ?: '—' }}</dd>
                    </dl>
                    <dl class="row mb-1">
                        <dt class="col-sm-4 fw-medium text-nowrap">
                            <i class="ti ti-certificate text-heading"></i>
                            <span class="fw-medium mx-2 text-heading">{{ __('License / Registration') }}</span>
                        </dt>
                        <dd class="col-sm-8">{{ $user->license_number ?: '—' }}</dd>
                    </dl>
                </div>
            </div>
        </div>
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-body">
                    <small class="card-text text-uppercase">{{ __('Office address') }}</small>
                    <p class="mt-3 mb-0 text-body">
                        @if ($user->office_address)
                            {!! nl2br(e($user->office_address)) !!}
                        @else
                            —
                        @endif
                    </p>
